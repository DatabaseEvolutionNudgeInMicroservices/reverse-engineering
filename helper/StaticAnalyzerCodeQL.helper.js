...
class StaticAnalyzerCodeQL extends StaticAnalyzer {
    ...
    initializesByList(list, language) {
        return new Promise((resolveAll, rejectAll) => {
            if (list !== undefined && list !== null && list.length !== 0 && language !== undefined && language !== null && language !== '' && LANGUAGES_SUPPORTED.includes(language)) {
                let promises = [];
                list.forEach(element => {
                    promises.push(new Promise((resolve, reject) => {
                        this.initializesByElement(element, language).then(result => {
                            resolve(result);
                        }).catch(error => {
    ...
    initializesByElement(element, language) {
        return new Promise((resolve, reject) => {
            if (element !== undefined && element !== null && element.length !== 0 && language !== undefined && language !== null && language !== '' && LANGUAGES_SUPPORTED.includes(language)) {
                let repositoryFolder = this.getRepositoryFolder(element);
                let codeQLRepositoryFolder = this.getCodeQLRepositoryFolder(element);

                // 2. Initialization

                try {
                    // CodeQL Database
                    exec('.' + FILE_SYSTEM_SEPARATOR + 'lib' + FILE_SYSTEM_SEPARATOR + 'codeql-cli' + FILE_SYSTEM_SEPARATOR + 'codeql database create --language=' + language + ' --source-root' + ' ' + repositoryFolder + ' ' + codeQLRepositoryFolder, {maxBuffer: 1024 * 1000 * 500}, (error, stdout, stderr) => {
                        if (error) {
                            reject(new AnalysisFail(error.message));
                        } else {
                            fs.mkdirSync(this.getResultCodeQLRepositoryFolder(element));
                            resolve(element);
                        }
                    });
                } catch (error) {
    ...
    identifyByList(list, language) {
        return new Promise((resolveAll, rejectAll) => {
            if (list !== undefined && list !== null && list.length !== 0 && language !== undefined && language !== null && language !== '' && LANGUAGES_SUPPORTED.includes(language)) {
                ...
                list.forEach(element => {
                    promises.push(new Promise((resolve, reject) => {
                        this.identifyByElement(element, language).then(result => {
                            resolve(result);
                        }).catch(error => {
                            reject(new AnalysisFail(error.message));
                        });
    ...
    identifyByElement(element, language) {
        return new Promise((resolve, reject) => {
            if (element !== undefined && element !== null && element.length !== 0 && language !== undefined && language !== null && language !== '' && LANGUAGES_SUPPORTED.includes(language)) {
                ...
                let codeQLRepositoryResultFile = this.getResultCodeQLRepositoryFile(element);
                try {

                    // 3. Identification

                    exec('.' + FILE_SYSTEM_SEPARATOR + 'lib' + FILE_SYSTEM_SEPARATOR + 'codeql-cli' + FILE_SYSTEM_SEPARATOR + 'codeql database analyze ' + codeQLRepositoryFolder + ' ' + codeQLQueryFolder + ' --format=csv --rerun --output=' + codeQLRepositoryResultFile, [], (error, stdout, stderr) => {
                        if (error) {
                            reject(new AnalysisFail(error.message));
                        } else {
                            resolve(element);
        ...
        return new Promise((resolve, reject) => {
            if (element !== undefined && element !== null && element.length !== 0 && language !== undefined && language !== null && language !== '' && LANGUAGES_SUPPORTED.includes(language)) {
                ...
                try {
                    ...
                    let folderPath = this.getRepositoryFolder(element);
        ...
        return new Promise(async (resolve, reject) => {
            if (list !== undefined && list !== null && list.length !== 0 && language !== undefined && language !== null && language !== '' && LANGUAGES_SUPPORTED.includes(language)) {
                ...
                for (let i = 0; i < list.length; i++) {
                    ...
                    let tdIdfRepositoryReferenceDocument = this.getConceptsByNLP(list[i].map(fragment => [fragment.tokens]).join(' '));
    ...
    getConceptsByNLP(data) {
        ...
        if (data !== undefined && data !== null && data !== "") {
            ...
            let lemmatizedConcepts = noiseFreeConcepts.map(concept => {
                ...
                let conceptSplitLemmatized = conceptSplit.map(conceptPart => winkNLPLemmatizer.noun(conceptPart));
    ...
    sortByTdIdf(data, referenceDocument) {
        if (data !== undefined && data !== null && data.length > 0) {
            if (referenceDocument !== undefined && referenceDocument !== null && referenceDocument !== "") {
                let unsortedData = [];
                let tfidf = new TfIdf();
    ...
    setCodeFragment(fragment, concepts, repositories) {
        if (fragment !== undefined && fragment !== null && concepts !== undefined && concepts !== null && repositories !== undefined && repositories !== null) {
            // Repository

            let repositoryPath = fragment.repository + '/';
            let repository = repositories.find(repository => repository.getLocation() === repositoryPath);
            ...
            let currentDirectoryPath = repositoryPath + (directories[0] !== undefined && directories[0].length !== 0 ? directories[0] + '/' : '');
            let currentDirectory = repository.getDirectories().find(directory => directory.getLocation() === currentDirectoryPath);
            ...
            for (let k = 1; k < directories.length; k++) {
                ...
                currentDirectory = currentDirectory.getDirectories().find(directory => directory.getLocation() === currentDirectoryPath);
                if (currentDirectory === undefined) {
                    currentDirectory = new Directory(currentDirectoryPath, [], []);
            ...
            let codeFragment = new CodeFragment(fragment.location, new Technology(fragment.type), new Operation(fragment.operation), new Method(fragment.method), new Sample(fragment.sample), concepts, fragment.heuristics, fragment.score);
     ...
     */
    getCodeQLRepositoryFolder(repository) {
        return this.getRepositoryFolder(repository) + CODEQL_FOLDER_NAME_SUFFIX;
    }
     ...
     * @return {String} The corresponding folder path.
     */
    getResultCodeQLRepositoryFolder(repository) {
        return this.getCodeQLRepositoryFolder(repository) + FILE_SYSTEM_SEPARATOR + RESULT_FOLDER_NAME;
    }
     ...
     */
    getResultCodeQLRepositoryFile(repository) {
        return this.getCodeQLRepositoryFolder(repository) + FILE_SYSTEM_SEPARATOR + RESULT_FOLDER_NAME + FILE_SYSTEM_SEPARATOR + RESULT_FILE_NAME;
    }
}
